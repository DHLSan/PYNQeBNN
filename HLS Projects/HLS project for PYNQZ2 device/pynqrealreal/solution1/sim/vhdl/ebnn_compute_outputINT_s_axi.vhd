...
entity ebnn_compute_outputINT_s_axi is
generic (
...
end entity ebnn_compute_outputINT_s_axi;
...
architecture behave of ebnn_compute_outputINT_s_axi is
    type states is (wridle, wrdata, wrresp, wrreset, rdidle, rddata, rdreset);  -- read and write fsm states
    signal wstate  : states := wrreset;
    signal rstate  : states := rdreset;
    signal wnext, rnext: states;
    constant ADDR_OUTPUT_R_BASE : INTEGER := 16#20#;
    constant ADDR_OUTPUT_R_HIGH : INTEGER := 16#3f#;
    ...
    -- memory signals
    signal int_output_r_address0 : UNSIGNED(2 downto 0);
    ...
    signal int_output_r_address1 : UNSIGNED(2 downto 0);
    ...
    signal int_output_r_shift  : UNSIGNED(1 downto 0);

    component ebnn_compute_outputINT_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component ebnn_compute_outputINT_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_output_r
int_output_r : ebnn_compute_outputINT_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 5,
     AWIDTH   => log2(5))
...
-- ----------------------- Memory logic ------------------
    -- output_r
    int_output_r_address0 <= SHIFT_RIGHT(UNSIGNED(output_r_address0), 2)(2 downto 0);
    int_output_r_ce0     <= output_r_ce0;
    int_output_r_we0     <= output_r_we0;
    int_output_r_be0     <= SHIFT_LEFT(TO_UNSIGNED(1, 4), TO_INTEGER(UNSIGNED(output_r_address0(1 downto 0))));
    int_output_r_d0      <= UNSIGNED(output_r_d0) & UNSIGNED(output_r_d0) & UNSIGNED(output_r_d0) & UNSIGNED(output_r_d0);
    int_output_r_address1 <= raddr(4 downto 2) when ar_hs = '1' else waddr(4 downto 2);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (output_r_ce0 = '1') then
                    int_output_r_shift <= UNSIGNED(output_r_address0(1 downto 0));
                end if;
            end if;
        end if;
    end process;
...
entity ebnn_compute_outputINT_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity ebnn_compute_outputINT_s_axi_ram;

architecture behave of ebnn_compute_outputINT_s_axi_ram is
